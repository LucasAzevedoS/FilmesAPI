...
public class FilmeController : ControllerBase
{
        ...
        JsonPatchDocument<UpdateFilmeDto> patch)
    {
        ...
        var filmeParaAtualiar = _mapper.Map<UpdateFilmeDto>(filme);

        patch.ApplyTo(filmeParaAtualiar, ModelState);

        if(!TryValidateModel(filmeParaAtualiar))
        {
            return ValidationProblem(ModelState);
        }

        _mapper.Map(filmeParaAtualiar, filme);
        _context.SaveChanges();
        return NoContent();
    }
